...
function addCuteCubes() {
	...
	for ( var i = 0; i < totalCubes; i ++ ) {
		...
		if ( i % 2 == 0 )  {
			...
			cube = new CuteCube( i, totalCubes, xRnd, zRnd, cuteCubeMesh, gamepadL, listener );

		}else {

			cube = new CuteCube( i, totalCubes, xRnd, zRnd, cuteCubeMesh, gamepadR, listener );
...
function render( timestamp ) {
	...
	for ( var i = 0; i < cubesArr.length; i ++ ) {
		...
		if ( i % 2 == 0 )  {
			...
			godPos = worldPosition.setFromMatrixPosition( gamepadL.matrixWorld );

		}else {

			godPos = worldPosition.setFromMatrixPosition( gamepadR.matrixWorld );

		}

		cubesArr[ i ].lookAt( new THREE.Vector3( godPos.x,godPos.y,godPos.z ) );
